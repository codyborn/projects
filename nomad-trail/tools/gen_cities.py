...
C = {
 ...
 'joshuatree':('Joshua Tree','USA','northamerica',34.13,-116.31,False,2,4,'hot',-8,0,['chili'],['boulder','hike'],'rock',"Bouldering by day, stars by night, a hot tub in the desert.",'cactus',{'sunburn':1.5,'nowifi':2.0}),
 'orangecounty':('Orange County','USA','northamerica',33.72,-117.83,True,2,5,'temperate',-8,0,['fishtacos'],['swim','surf','bands'],'wave',"Home. Family, laundry that someone else folds, and the Pacific as the front door.",'sun',{'goodday':1.5}),
 ...
 'lapaz':('La Paz','Mexico','mexico',24.14,-110.31,False,2,5,'hot',-7,0,['tacos','bajatacos'],['swim','hike'],'wave',"Whale sharks, a rental car, the good kind of quiet.",'whale',{'foodpoisoning':1.6}),
 'laventana':('La Ventana','Mexico','mexico',24.05,-109.99,True,4,12,'hot',-7,0,['bajatacos','tacos'],['kite','swim','yoga'],'gust',"Kiteboarding town. Wind every afternoon, NYE on the beach, nothing to do until 2 pm.",'kite',{'windday':2.5,'foodpoisoning':1.4}),
 'roatan':('Roatan','Honduras','mexico',16.32,-86.53,True,7,21,'hot',-6,0,['baleada','conchsoup'],['swim','yoga','bands'],'wave',"Five weeks on an island of biohackers, reef, and one road.",'reef',{'seaurchin':2.0,'foodpoisoning':1.3,'mosquito':1.2}),
 ...
 'hyeres':('Hyeres','France','europe',43.12,6.13,True,10,26,'hot',1,0,['bouillabaisse','ratatouille'],['swim','trailrun','kite','bands'],'mosquito',"Twenty-six days on the Riviera. Bread is the answer to most things.",'palm',{'mosquito':3.0,'goodday':1.5}),
 ...
 'dakhla':('Dakhla','Morocco','africa',23.71,-15.94,True,7,21,'hot',1,0,['tagine','couscous'],['kite','swim','yoga','bands'],'gust',"Three weeks on a lagoon in the Sahara. The French have watches.",'dune',{'windday':3.0,'foodpoisoning':1.5,'sunburn':2.0}),
...
import math
def km(a,b):
    la,lo,lb,lob=map(math.radians,(C[a][3],C[a][4],C[b][3],C[b][4])); return 6371*math.acos(min(1,math.sin(la)*math.sin(lb)+math.cos(la)*math.cos(lb)*math.cos(lo-lob)))
E=[e for e in E if abs(wrap(C[e[0]][4]-C[e[1]][4]))<=130 or print('drop long-haul', e[0], e[1])]
for e in E:
    ...
    for x,y in ((a,b),(b,a)):
        ...
        tz = abs(C[x][9]-C[y][9])
        if t2=='flight': d2 = 2 if (tz > 6 or km(x,y) > 8000) else 1   # long-haul = 2 days, everything else 1
...
for cid,(name,country,region,lat,lon,hero,mins,sug,climate,tz,alt,dishes,acts,hazard,blurb,icon,ew) in C.items():
    ...
    mins=max(mins, round(sug*0.8)); sug=max(3, round(sug*1.25)); mins=max(2, round(mins*1.25)) if cid!='orangecounty' else 2   # x1.25 = the 2026-09-24 balance tuning
...
D = [
 ...
 ('bajatacos','Baja fish tacos','lapaz',['white fish','tortilla','cabbage','lime','crema'],S(('chop',4),('flip',3),('season',2),('pour',2)),9,10),
 ('conchsoup','Sopa de caracol','roatan',['conch','coconut milk','plantain','cilantro'],S(('chop',5),('stir',5),('pour',2)),9,9),
