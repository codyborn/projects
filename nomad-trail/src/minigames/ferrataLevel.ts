...
export type LedgeKind = 'normal' | 'thin' | 'anchor';
...
export const FERRATA = {
  ...
  steer: 150,                         // horizontal speed while holding a side (px/s)
  rise: 72,                           // vertical gap between consecutive ledges
...
/** Ledges from the ground (y = 0 is the start ledge; y decreases upward). Staggered so a straight bounce misses; every 4th is an anchor. No crumble ledges. */
export function genLedges(seed = 1): Ledge[] {
  ...
  for (let i = 1; i <= ledges; i++) {
    const kind: LedgeKind = i % 4 === 0 ? 'anchor' : r() < 0.25 ? 'thin' : 'normal';
    const w = kind === 'anchor' ? 84 : kind === 'thin' ? 40 : 62;   // nothing narrower than 40 px
...
export function validateLedges(ls: Ledge[]): string[] {
  ...
  for (let i = 1; i < ls.length; i++) { const a = ls[i - 1], b = ls[i]; if (a.y - b.y !== FERRATA.rise) errs.push(`gap ${i}: ${a.y - b.y}`); const d = Math.abs((a.x + a.w / 2) - (b.x + b.w / 2)); if (d > reach + b.w / 2) errs.push(`ledge ${i} too far: ${d.toFixed(0)} > ${(reach + b.w / 2).toFixed(0)}`); if ((b as any).kind === 'crumble') errs.push(`ledge ${i} is a crumble ledge`); if (b.x < FERRATA.wallPad || b.x + b.w > FERRATA.width - FERRATA.wallPad) errs.push(`ledge ${i} in wall`); }
  return errs;
}
