...
| `Airport` | AirportScene | `{ gate?: string }` e.g. 'B56' | GATE DASH, Temple-Run style: the taxi died, run to your gate down a pseudo-3D terminal. Swipe left/right (or tap the left/right third) to change lane, swipe up (or tap the middle) to jump; arrows on desktop. Dodge travellers and cleaning carts by lane, jump bags and rope barriers, ride moving walkways for a boost. Every ~4.5 s the corridor forks under two signs with gate ranges: take the side containing your gate. Five forks narrow it (A–C / D–F → the letter → 1–50 / 51–99 → the quarter → the door itself across the three lanes). Wrong side or the middle at a fork = WRONG WAY, U-turn, 3 s lost, fork repeats. Boarding closes after 40 s of play. | success: 100 − 8 per wrong turn − 1 per collision (−4 each beyond two), min 40; PERFECT with no wrong turns and ≤ 2 collisions. Boarding closed = FAILED, ≤ 45 by forks passed. |
