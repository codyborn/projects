...
export class WorkoutScene extends Phaser.Scene {
  ...
  private ferrata() {
    ...
    let cliffY = 0; const climber = this.add.rectangle(W / 2, 470, 16, 26, PAL.sun0).setDepth(6); const cara = this.add.graphics().setDepth(7); ((this as any)._scoreNow as Record<string, () => number>).ferrata = () => (clipped / total) * 100 * (tries ? clipped / tries : 1);
  ...
  private trailrun() {
    ...
    this.loop(() => { const dt = 0.016; const el = (this.time.now - t0) / 1000; const rp = 1 + 0.6 * clamp(el / dur, 0, 1); dist += spd * rp * dt; if (el > nextRock) { rocks.push({ x: W + 20, w: Phaser.Math.Between(14, 26), h: Phaser.Math.Between(14, 28), hit: false }); nextRock = el + Phaser.Math.FloatBetween(0.9, 1.7) / (this.frame.speed * rp); }
      ...
      for (const r of rocks) { r.x -= spd * rp * dt; this.g.fillStyle(r.hit ? PAL.red : PAL.gray1).fillRect(r.x, groundY - r.h, r.w, r.h); if (!r.hit && r.x < 88 && r.x + r.w > 72 && y > groundY - r.h + 4) { r.hit = true; hits++; this.frame.shake(120, 0.006); this.frame.flash(PAL.red, 60); } }
