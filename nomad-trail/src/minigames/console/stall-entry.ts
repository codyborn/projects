...
game.events.once('ready', () => {
  ...
  game.scene.start(MINIGAME_KEYS.carryon, { energy: 100, difficulty: Number(q.get('diff') || 0.5), payload: { game: q.get('game') || 'tetris', city: 'lisbon', cityName: 'Lisbon', seed: Number(q.get('seed') || 5) },
    onDone: (r) => { calls++; out.textContent = JSON.stringify({ virtualSeconds: game.getTime() / 1000, wall: (performance.now() - t0) / 1000, calls, result: r, cardSeenAt: (window as any).__cardSeenVirtual, errors }); document.title = 'STALL_DONE'; } } as MinigameLaunch);
});
