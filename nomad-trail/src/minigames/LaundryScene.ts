...
export class LaundryScene extends Phaser.Scene {
  ...
  private step(dt: number) {
    ...
    this.meter.set(this.elapsed / this.dur, PAL.sun2); this.frame.setTimer(`${Math.max(0, Math.ceil(this.dur - this.elapsed))}s  x${this.speedMul().toFixed(1)}`); this.frame.setProgress(`${this.right} sorted${this.wrong ? ` · ${this.wrong} wrong` : ''}`);
    if (this.elapsed >= this.dur) { this.ended = true; const base = this.total > 1 ? (this.right / (this.total - 1)) * 100 : 0; this.frame.finish(this.pinkTint ? Math.min(base, 60) : base); }
  }
