...
export class LaundryScene extends Phaser.Scene {
  ...
  private items: { x: number; y: number; vx: number; white: boolean; color: number; shape: number; done: boolean; result?: boolean }[] = []; private right = 0; private wrong = 0; private total = 0; private elapsed = 0; private tick?: Phaser.Time.TimerEvent; private downX = 0; private pinkTint = false; private ended = false; private bag: boolean[] = [];
  constructor() { super(MINIGAME_KEYS.laundry); }
  init(data: any) { this.launch = normalizeLaunch(data); this.items = []; this.right = 0; this.wrong = 0; this.total = 0; this.elapsed = 0; this.pinkTint = false; this.ended = false; this.bag = []; }
  create() {
  ...
  private step(dt: number) {
    ...
    const g = this.g; g.clear(); g.fillStyle(PAL.gray0).fillRect(0, 40, W, 6); for (const it of this.items) { const tint = this.pinkTint && it.white ? PAL.pink : it.color; g.fillStyle(it.white ? PAL.gray1 : PAL.ink); this.shape(g, it.shape, it.x, it.y, 1.15); g.fillStyle(tint); this.shape(g, it.shape, it.x, it.y, 1); }
