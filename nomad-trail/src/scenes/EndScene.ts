...
export class EndScene extends Phaser.Scene {
  ...
  create() {
    ...
    const t = txt(this, 180, 56, title, 26, color).setOrigin(0.5).setScale(1.6).setAlpha(0); this.tweens.add({ targets: t, scaleX: 1, scaleY: 1, alpha: 1, duration: 500, ease: 'Back.Out' });
    txt(this, 180, 118, end.text, 10, PAL.white, { align: 'center', wrap: 300 }).setOrigin(0.5);
    const p = new Panel(this, 24, 180, 312, 230, { fill: PAL.night1 });
    const rows: [string, string][] = [['Days', `${run.day} / 365`], ['Cities', `${run.visited.length}`], ['Health', `${Math.round(run.health)}`], ['Mood', `${Math.round(run.mood)}`], ['Items lost', `${run.lostItems.length}`], ['Coffee mornings', `${run.coffeeMornings}`], ['Achievements', Sim.visibleAchievements(run).length ? Sim.visibleAchievements(run).join(', ') : 'none']];
    ...
    if (end.kind !== 'win' && this.cause(run, end) !== end.text) txt(this, 180, 430, `Cause: ${this.cause(run, end)}`, 9, PAL.gray2, { align: 'center', wrap: 300 }).setOrigin(0.5);
