...
import { Sim, getSettings, putSettings, putRun, Data } from '../ui/simBridge';
...
export class TitleScene extends Phaser.Scene {
  ...
  create() {
    ...
    const logo = txt(this, 180, 130, 'NOMAD TRAIL', 32, PAL.white).setOrigin(0.5);
    this.tweens.add({ targets: logo, y: 134, duration: 1800, yoyo: true, repeat: -1, ease: 'Sine.InOut' });
    txt(this, 180, 166, 'Circumnavigate the planet in 365 days.\nPack light. Trust no kettle.', 10, PAL.sun3, { align: 'center' }).setOrigin(0.5);
    ...
    txt(this, 180, 604, `Based on actual events.${settings.runs ? `  Runs: ${settings.runs}  Best: ${settings.bestScore}` : ''}`, 8, PAL.gray1, { align: 'center' }).setOrigin(0.5);
    txt(this, 180, 620, 'A Citizen Earth production', 8, PAL.gray0).setOrigin(0.5);
  }
  ...
  private newRun(hasSave: boolean) {
    const dim = dimmer(this, 0.7); const p = new Panel(this, 20, 160, 320, 330, { fill: PAL.night1 });
    const items: Phaser.GameObjects.GameObject[] = [dim, p];
    items.push(txt(this, 180, 180, hasSave ? 'This replaces your saved run.' : 'A new year begins.', 10, PAL.sun1).setOrigin(0.5) as any);
    items.push(txt(this, 180, 210, 'Start and finish in', 12, PAL.gray2).setOrigin(0.5) as any);
    let start = 'miami', dir: 'east' | 'west' = 'east';
    const starts = Data.cities.filter(c => c.id === 'miami' || c.id === 'newyork'); if (!starts.length) starts.push(Data.cities[0]);
    const sb: Button[] = []; starts.forEach((c, i) => { const b = new Button(this, 100 + i * 160, 248, c.name.toUpperCase(), () => { start = c.id; sb.forEach((x, j) => x.setAlpha(j === i ? 1 : 0.5)); }, { w: 140, fill: PAL.sea0 }); sb.push(b); items.push(b); }); sb.forEach((x, j) => x.setAlpha(j === 0 ? 1 : 0.5));
    items.push(txt(this, 180, 300, 'Heading', 12, PAL.gray2).setOrigin(0.5) as any);
    const db: Button[] = []; (['east', 'west'] as const).forEach((d, i) => { const b = new Button(this, 100 + i * 160, 338, d === 'east' ? 'EAST  →' : '←  WEST', () => { dir = d; db.forEach((x, j) => x.setAlpha(j === i ? 1 : 0.5)); }, { w: 140, fill: PAL.dusk0 }); db.push(b); items.push(b); }); db[1].setAlpha(0.5);
    items.push(new Button(this, 180, 420, 'START PACKING', () => { const s = Sim.createRun(Date.now() % 1e9, start, dir); putRun(this, s); this.scene.start('Pack'); }, { w: 240, fill: PAL.sun0 }));
    items.push(new Button(this, 180, 470, 'back', () => items.forEach(i => i.destroy()), { w: 120, h: 36, size: 10, fill: PAL.night2 }));
  }
}
