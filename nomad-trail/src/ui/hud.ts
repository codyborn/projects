...
export class Hud extends Phaser.GameObjects.Container {
  ...
  constructor(scene: Phaser.Scene) {
    ...
    this.day = txt(scene, 8, 4, '', 12, PAL.sun2); this.add(this.day as any);
    ...
    this.clothes = txt(scene, 352, 4, '', 8, PAL.gray2).setOrigin(1, 0); this.add(this.clothes as any);
    this.weight = txt(scene, 8, 48, '', 8, PAL.gray2); this.add(this.weight as any);
    this.badges = txt(scene, 352, 48, '', 8, PAL.sun1).setOrigin(1, 0); this.add(this.badges as any);
  ...
  refresh(s: RunState) {
    ...
    this.bars.health.set(s.health); this.bars.energy.set(s.energy); this.bars.mood.set(s.mood);
