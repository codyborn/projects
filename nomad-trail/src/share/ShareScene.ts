...
export class ShareScene extends Phaser.Scene {
  ...
  init(d?: { onBack?: () => void; back?: string }) { this.onBack = d?.onBack; this.backKey = d?.back; }
  async create() {
    ...
    btn(GAME_W / 2 + 110, 'BACK', () => { const cb = this.onBack; const back = this.backKey; this.scene.stop(); if (cb) cb(); else this.scene.start(back ?? 'End'); });
