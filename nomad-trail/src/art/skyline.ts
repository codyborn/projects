...
const CITY: Record<string, Drawer> = {
  ...
  iguazu: (ctx, w, hy, L, c, win, r) => { if (L === 'far') { // green ridge with a recorded profile so the pines stand on it
      ...
      for (let i = 0; i < 8; i++) { const tx = r.int(4, w - 4), th = r.int(8, 16), base = prof[tx] + 1; for (let y = 0; y < th; y++) { const hw = Math.max(0, Math.round((y / th) * 4)); R(ctx, tx - hw, base - th + y, hw * 2 + 1, 1, PAL.grass0); } } }
