...
const cities = process.argv[2] ? process.argv[2].split(',') : ['patagonia', 'iguazu', 'brussels', 'hyeres', 'antibes', 'edinburgh', 'highlands', 'reykjavik', 'dakhla', 'bangkok', 'hongkong', 'tokyo', 'minakami', 'kathmandu'];
fs.mkdirSync('e2e/layout/art', { recursive: true });
const tods = process.argv[4] ? process.argv[4].split(',') : ['day', 'dusk'];
for (const c of cities) for (const tod of tods) { await pg.evaluate((id, tod) => { const sc = window.__nomad.game.scene.getScene('Title'); if (window.__sk) { try { window.__sk.destroy(); } catch {} } window.__sk = window.__nomadArt.skylineAt(sc, id, tod, 0, 100, 360, 150); }, c, tod); await new Promise(r => setTimeout(r, 120)); await pg.screenshot({ path: `e2e/layout/art/${c}-${tod}.png`, clip: { x: 0, y: 100, width: 360, height: 150 } }); }
for (const d of (process.argv[3] ? process.argv[3].split(',') : ['frites', 'moules'])) fs.writeFileSync(`e2e/layout/art/dish-${d}.png`, Buffer.from((await pg.evaluate(id => window.__nomad.review.dishPng(id), d)).split(',')[1], 'base64'));
