...
function batch(style: PackStyle, n = N) {
  ...
  const mean = outs.reduce((a, o) => a + o.day, 0) / n; const conts = outs.reduce((a, o) => a + o.continents, 0) / n; const five = outs.filter(o => o.continents === 5).length; const money = outs.reduce((a, o) => a + o.money, 0) / n;
  console.log(`\n== ${style} pack, ${n} runs ==`);
  ...
  console.log('causes', causes, ` mean end money $${money.toFixed(0)}`);
