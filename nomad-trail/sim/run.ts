...
function batch(style: PackStyle, n = N) {
  ...
  const mean = outs.reduce((a, o) => a + o.day, 0) / n;
  console.log(`\n== ${style} pack, ${n} runs ==`);
  console.log(`win ${wins} (${Math.round(100 * wins / n)}%)  fail ${n - wins} (${Math.round(100 * (n - wins) / n)}%)  mean end day ${mean.toFixed(0)}  mean cities ${(outs.reduce((a, o) => a + o.cities, 0) / n).toFixed(1)}`);
